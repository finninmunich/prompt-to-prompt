...
ldm_stable = StableDiffusionPipeline.from_pretrained("/home/turing/GDD/hugging_face/CompVis/stable-diffusion-v1-4", use_auth_token=MY_TOKEN, scheduler=scheduler).to(device)
try:
...
def run_and_display(prompts, controller, latent=None, run_baseline=False, generator=None, uncond_embeddings=None, verbose=True):
    ...
    global index
    index += 1
    ptp_utils.view_images(images, index)
    return images, x_t
...
image_path = "./jidu/26.jpg"
prompt = "A car driving down a street next to a traffic light."
(image_gt, image_enc), x_t, uncond_embeddings = null_inversion.invert(image_path, prompt, offsets=(0,0,200,0), verbose=True)
cv2.imwrite("dst.png", image_gt)
cv2.imwrite("dst1.png", image_enc)
...
# ptp_utils.view_images([image_gt, image_enc, image_inv[0]], index)
...
#'''
prompts = ["A train is sitting on the tracks in the grass in the daytime.",
           "A car is sitting on the tracks in the grass in the daytime."
        ]

cross_replace_steps = {'default_': .8,}
self_replace_steps = .5
blend_word = ((('train',), ("car",))) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
eq_params = {"words": ("car",), "values": (2,)} # amplify attention to the word "tiger" by *2 
...
prompts = ["A train is sitting on the tracks in the grass in the daytime.",
           "A train is sitting on the tracks in the grass in the snow."
        ]

cross_replace_steps = {'default_': .8, }
self_replace_steps = .6
blend_word = ((('train',), ("train",))) # for local edit
eq_params = {"words": ("snow", ), "values": (2,)}  # amplify attention to the words "silver" and "sculpture" by *2 
...
# prompts = ["A car driving down a street next to a traffic light.",
#            "A car driving down a rainy street next to a traffic light."
#         ]

# cross_replace_steps = {'default_': .8, }
# self_replace_steps = .7
# blend_word = None
# eq_params = {"words": ("rainy",  ), "values": (5,)}  # amplify attention to the word "watercolor" by 5
