...
MY_TOKEN = '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'
LOW_RESOURCE = False
NUM_DDIM_STEPS = 50
GUIDANCE_SCALE = 7.5
MAX_NUM_WORDS = 77
device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
ldm_stable = StableDiffusionPipeline.from_pretrained("models/CompVis/stable-diffusion-v1-4", use_auth_token=MY_TOKEN,
...
def load_512(image_path, left=0, right=0, top=0, bottom=0):
    ...
    #image = np.array(Image.fromarray(image).resize((720,1280)))
    return image
...
class NullInversion:
    ...
    def next_step(self, model_output: Union[torch.FloatTensor, np.ndarray], timestep: int,
        ...
        alpha_prod_t = self.scheduler.alphas_cumprod[timestep] if timestep >= 0 else self.scheduler.final_alpha_cumprod # t
        alpha_prod_t_next = self.scheduler.alphas_cumprod[next_timestep] # t+1
        beta_prod_t = 1 - alpha_prod_t
    ...
    def image2latent(self, image):
        with torch.no_grad():
            ...
            if type(image) is torch.Tensor and image.dim() == 4:
                ...
                latents = latents * 0.18215 #scaling factor is using here
        return latents
    ...
    def ddim_loop(self, latent):
        ...
        all_latent = [latent] # z_0
        latent = latent.clone().detach()
        for i in range(NUM_DDIM_STEPS):
            t = self.model.scheduler.timesteps[len(self.model.scheduler.timesteps) - i - 1]
            noise_pred = self.get_noise_pred_single(latent, t, cond_embeddings) # pivot ddim inversion whose omega is 1
    ...
    def ddim_inversion(self, image):
        ...
        return image_rec, ddim_latents # ddim_latents is a list
    ...
    def invert(self, image_path: str, prompt: str, offsets=(0, 0, 0, 0), num_inner_steps=10, early_stop_epsilon=1e-5,
               verbose=False):
        self.init_prompt(prompt) # assigning--> self.context = torch.cat([uncond_embeddings, text_embeddings])  self.prompt = prompt
        ptp_utils.register_attention_control(self.model, None)
        image_gt = load_512(image_path, *offsets)
        if verbose:
            print("DDIM inversion...")
        image_rec, ddim_latents = self.ddim_inversion(image_gt) # ddim_latents = [z_0, z_1, ..., z_T]
        if verbose:
...
def run_and_display(prompts, controller, latent=None, run_baseline=False, generator=None, uncond_embeddings=None,
    ...
    if verbose:
        ptp_utils.view_images(images,imgname=imgname)
    return images, x_t


if __name__ == '__main__':
    null_inversion = NullInversion(ldm_stable)
    filename="1681277828417549992_fisheye_left.png"
    image_path = f"./example_images/jidu/Sunny/{filename}"
    prompt = "a realistic photo taken in sunny day"
    (image_gt, image_enc), x_t, uncond_embeddings = null_inversion.invert(image_path, prompt, offsets=(0, 0, 0, 0),
                                                                          verbose=True)
    #prompts = [prompt]
    # controller = AttentionStore()
    # # image_inv_wo_nt, _ = run_and_display(prompts, EmptyControl(), run_baseline=True, latent=x_t,
    # #                                  uncond_embeddings=None, verbose=True,
    # #                                  imgname=None)
    # image_inv, x_t = run_and_display(prompts, controller, run_baseline=False, latent=x_t,
    #                                  uncond_embeddings=uncond_embeddings, verbose=True,
    #                                  imgname=None)
    # print("showing from left to right: the ground truth image, "
    #       "the vq-autoencoder reconstruction, the null-text inverted image")
    # ptp_utils.view_images([image_gt, image_enc, image_inv[0],image_inv_wo_nt[0]],imgname='./result_images/jidu/sunny2evening/gt_vq_nt_wont')
    # show_cross_attention(controller, 16, ["up", "down"])
    prompts = ["a realistic photo taken in sunny day",
               "a realistic photo taken in evening day"
               ]

    cross_replace_steps = {'default_': .8, }
    self_replace_steps = .4
    blend_word =None
    eq_params = {"words": ("evening",), "values": (2,)}  # amplify attention to the word "tiger" by *2
    ...
    images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t,
                                uncond_embeddings=uncond_embeddings,imgname=f"./result_images/jidu/sunny2evening/{filename}")
    ...
    # eq_params = {"words": ("silver", 'sculpture',),
    #              "values": (2, 2,)}  # amplify attention to the words "silver" and "sculpture" by *2
    #
    # controller = make_controller(prompts, False, cross_replace_steps, self_replace_steps, blend_word, eq_params)
    # images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings,imgname="cat_resutls_2")
    # prompts = ["a cat sitting next to a mirror",
    ...
    # controller = make_controller(prompts, False, cross_replace_steps, self_replace_steps, blend_word, eq_params)
    # images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t,
    #                             uncond_embeddings=uncond_embeddings,imgname="cat_resutls_3")